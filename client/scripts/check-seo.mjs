 ...
 *
 * Смотрит ГОТОВУЮ выдачу, а не исходники: файлы, которые запёк postbuild. Ловит то, что глазами не
...
const PROJECT = 'Ascora-Education';
const HOST = 'www.ascora.education';
const ROOT_REL = '../dist';
const SITEMAP = 'sitemap.xml';

/** [путь, запечённый файл, ожидания].
 *
 *  FAQPage здесь намеренно НЕ в статике: <div id="root"> в этих файлах пуст,
 *  а разметка FAQ без видимых вопросов недействительна для краулеров,
 *  которые не исполняют JS. Её добавляет <SeoHead> на клиенте. Поэтому
 *  проверка вопросов тут просто не срабатывает — и это правильно. */
const PAGES = [
  ['/',                           'index.html',                            { indexable: true, lang: 'ru' }],
  ['/obuchenie-za-rubezhom',      'obuchenie-za-rubezhom/index.html',      { indexable: true, lang: 'ru' }],
  ['/letnie-lagerya-za-rubezhom', 'letnie-lagerya-za-rubezhom/index.html', { indexable: true, lang: 'ru' }],
  ['/leto-lager-v-turcii',        'leto-lager-v-turcii/index.html',        { indexable: true, lang: 'ru' }],
];
/* ─────────────────────────────────────────────────────────────────────────
